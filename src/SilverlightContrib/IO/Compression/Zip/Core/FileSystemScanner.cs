...
namespace SilverlightContrib.IO.Compression.Zip.Core
{
	#region EventArgs
	/// <summary>
	...
	public class FileSystemScanner
	{
		...
		void ScanDir(string directory, bool recurse)
		{
			...
			try
			{
			    string[] names = (string[])Directory.EnumerateFiles(directory);
				bool hasMatch = false;
			...
			if ( alive_ && recurse ) {
				try {
					string[] names = (string[])Directory.EnumerateDirectories(directory);
